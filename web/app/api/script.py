...
@app.route('/api/carga-otros',methods=['POST'])
def carga_address_otras():
    ...
    dataClientDF = pandas.DataFrame(data=matriz,columns=colnames)
    if(dataClient.startswith("0")):data = pandas.DataFrame(dataClientDF[2:],columns=colnames) #sin preprocesado
    else: data = dataClientDF

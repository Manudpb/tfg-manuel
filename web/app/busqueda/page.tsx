...
const queries = [
  ...
  function buildQuery(selectedColumn: string, selectedConsulta: string, selectedTable: string,condicion:string) {
    ...
    if(getConsultaNum != 8 && getConsultaNum !== undefined && getTablesNum !== undefined && getCol !== undefined){
      setConsulta(buildQuery(columnas[getCol],queries[getConsultaNum],getTables[getTablesNum],getCondicion))
    }
    const dataToSend = {'consulta':getConsulta}
    ...
    }).then(response =>{ setSuccess(true); response.json().then(data => setRuta(data.ruta)) }).catch(error =>{console.error('Error:', error);setError(true)
  })
}
  ...
  useEffect(() => {
    ...
    return(
      <main className="flex flex-col space-y-10 items-center justify-between text-black">
        <div className="flex items-center justify-between space-x-10">
          ...
          <div className="flex flex-col space-y-5 ">
            ...
            {getConsultaNum === 8 ?<input onChange={(e)=>setConsulta(e.target.value)} className="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-3 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Introduce la consulta..." type="text"/>:<></> }
          </div>
